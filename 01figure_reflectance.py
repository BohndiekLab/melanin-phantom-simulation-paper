...
from simulation_code.iad_sim import iad_sim_layers
from simulation_code.colour_sim import get_colour
...
def get_layer_muas(tissue_model, wavelengths=WAVELENGTHS):
    ...
    results = {}
    for l in tissue_model.tissues:
        results[l.name] = l.absorption(WAVELENGTHS)
    return results
...
for mvf in mvfs:
    tissue = load_model_from_excel("simulation_specification/forearm_tissue_model.xlsx", nx, ny, nz, dx, 
                                   tissue_cz=-nz*dx/3, 
...
np.savez("data/reflectance/intermediate_results", reflectances=reflectances, 
         ita=itas, reflectances_685=reflectances_685,
         rgbs=rgbs, wavelengths=WAVELENGTHS, fp=fps,
         epidermis_mua = epidermis_mua, background_mua = background_mua,
