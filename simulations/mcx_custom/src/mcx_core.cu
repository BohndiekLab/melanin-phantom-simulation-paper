...
void mcx_run_simulation(Config* cfg, GPUInfo* gpu) {
    ...
    // MCX_FPRINTF(cfg->flog, "TOM WOZ HERE: mm = %f, p1 = %f, %f, %f, %f, TYPE=%d, RC0 = %f, minstep = %f, vx=%f, vy=%f, vz=%f, vw=%f; x=%f, y=%f, z=%f, w=%f\n", (float)cfg->unitinmm, (float)(cfg->srcparam1.x), (float)(cfg->srcparam1.y), (float)(cfg->srcparam1.z), (float)(cfg->srcparam1.w), (int)cfg->srctype, (float) R_C0, (float)minstep, (float)cfg->srcdir.x, (float)cfg->srcdir.y, (float)cfg->srcdir.z, (float)cfg->srcdir.w, (float)cfg->srcpos.x, (float)cfg->srcpos.y, (float)cfg->srcpos.z, (float)cfg->srcpos.w);
